	...
	try {
		...
		const usersResult: QueryResult<{id: string; username: string}> =
			await pool.query(`
				...
				VALUES 
					('alice', 'alice@example.com', 'https://alice.png', 4),
					('bob', 'bob@example.com', 'https://bob.png', 3),
					('charlie', 'charlie@example.com', 'https://charlie.png', 5)
		...
		if (usersResult.rows.length === 0) {
			...
			userMap = Object.fromEntries(
				allUsers.rows.map((row) => [row.username, row.id])
			);
		} else {
			userMap = Object.fromEntries(
				usersResult.rows.map((row) => [row.username, row.id])
